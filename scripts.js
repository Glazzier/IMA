document.addEventListener("DOMContentLoaded", function () {
  ...
  if (applyForm) {
    applyForm.addEventListener("submit", function (event) {
      ...
      sendEmbedToWebhook(formData, webhookURL, "Aplicación para Moderador");
    });
  }
  ...
  if (gdExamForm) {
    gdExamForm.addEventListener("submit", function (event) {
      ...
      sendEmbedToWebhook(formData, webhookURL, "Respuestas del Examen GD");
    });
  }

  function sendEmbedToWebhook(formData, webhookURL, title) {
    const embed = {
      title: title,
      fields: [],
    };
